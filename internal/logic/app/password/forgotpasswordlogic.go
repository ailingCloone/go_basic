...
func (l *ForgotPasswordLogic) ForgotPassword(req *types.PostForgotPasswordAppReq) (resp *types.SuccessResponse, err error) {
	var c config.Config
	...
	username, err := UserVerification(req.From, req.Email, l.ctx, conn)
	if err != nil {
		return nil, err
	}
	...
	otpRecord, sendStatus, err := otpFunc.GenerateEmailOtpRecord(l.ctx, conn, req.Email, auth, from, c.EmailOtpExpire, body, subject)
	if err != nil {
		return nil, err
	}
...
func UserVerification(from, email string, ctx context.Context, conn sqlx.SqlConn) (username string, err error) {
	if from == "staff" {
		exists, err := user.CheckStaff(ctx, conn, "EMAIL", email)
		if err != nil {
			return "", err
		}
	...
	if from == "customer" {
		exists, err := user.CheckCustomer(ctx, conn, "EMAIL", email)
		if err != nil {
			return "", err
		}
		username = exists.Username //customer table has 'fullname' and 'username'
