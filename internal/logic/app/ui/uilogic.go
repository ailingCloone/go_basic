...
func (l *UiLogic) Ui(req *types.GetUiReq) (resp *types.SuccessResponse, err error) {
	...
	oauthResult, err := uiModel.FindOne(context.Background(), 15)
	if err != nil {
		return nil, err
	}

	header, err := getUiHeader(uiModel, oauthResult.UiHeaderId.Int64)
	if err != nil {
		return nil, err
	}

	uiSummaryList, err := uiModel.FindUiSummary(context.Background(), oauthResult.Id)
	if err != nil {
		return nil, err
	}
	...
	for _, summary := range uiSummaryList {
		...
		case "ui_content":
			uiContentList, err := uiModel.FindUiContent(l.ctx, oauthResult.Id)
			if err != nil {
				return nil, err
			}
			...
			for _, content := range uiContentList {
				if content.Id == summary.ReferId {
					titles = GetTitle(summary, uiModel, l.ctx, oauthResult.Id)
		...
		case "ui_button":
			...
			uiButtonList, err := uiModel.FindUiButton(l.ctx, oauthResult.Id)
			if err != nil {
				return nil, err
			}
