...
func (l *DeleteLogic) Delete(req *types.PostATDeleteReq) (resp *types.SuccessResponse, err error) {
	...
	agreeTerm.Active = 0
	data := agreeTerm
	if err := agreeTermModel.DeleteByGuid(l.ctx, data); err != nil {
		return responseglobal.GenerateResponseBody(false, "Failed to delete Agree term.", map[string]interface{}{}), nil
	}
	...
	return responseglobal.GenerateResponseBody(true, "AgreeTerm deleted successfully.", dataResp), nil
}

func DeleteConvertChangesActivityLog(data *agree_term.AgreeTerm, previousData *agree_term.AgreeTerm) (jsonData []byte, err error) {
