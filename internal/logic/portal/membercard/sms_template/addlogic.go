...
func (l *AddLogic) Add(req *types.PostSTAddReq) (resp *types.SuccessResponse, err error) {
	...
	result, err := smsTemplateModel.Insert(context.Background(), data)
...
func ConvertChangesActivityLog(data *sms_template.SmsTemplate) (jsonData []byte, err error) {

	statusAfter := strconv.FormatInt(data.Status, 10)
	...
	changes := map[string]activity_log.Change{
		...
		"status": {
			Before: "",          // Before value for Description
			After:  statusAfter, // After value for Description
		},
		"created": {
